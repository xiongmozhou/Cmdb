...
class AssetView(View):
    ...
    def post(self, request, *args, **kwargs):
       ...
       """
        disk_obj = models.Disk.objects.values()
        for disk_list in disk_obj:
            print(disk_list)
        # if server_info['disk']['data']  not in disk_obj:
        #     models.Disk.objects.create(server_info['disk']['data'])
        #     log_str = '[新增硬盘]插槽为{slot};容量为{capacity};硬盘类型为{pd_type};型号为{model}'.format(**server_info['disk']['data'])
        #     print(log_str)
